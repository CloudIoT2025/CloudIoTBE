...
exports.getUserInfo = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT id, username as name FROM users LIMIT 1');
        if (rows.length > 0) {
...
exports.getTodayResults = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('SELECT calories_fitbit AS calories, goal FROM fitbit_data WHERE date = ? ORDER BY created_at DESC LIMIT 1', [today]);
        if (rows.length > 0) {
...
exports.getWeeklyData = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT calories_fitbit AS calories, date AS createdAt FROM fitbit_data ORDER BY date ASC LIMIT 7');
