...
async def async_test(client, data, rsp_id):
    ...
    result = client.publish(
        f"move/end/" + rsp_id, str(round(cal * ratio / 100, 2)) + "," + userId, qos=2
    )
    if result.rc != mqtt_client.MQTT_ERR_SUCCESS:
        logger.error(f"Publish failed (response): rc={result.rc}")
    ...
    if msg.topic == "move/start/" + RSPID:
        if latest_async_data:
            ...
            asyncio.run_coroutine_threadsafe(
                async_test(client, data, RSPID), async_loop
            )
        else:
...
def run():
    ...
    while True:
        try:
            ...
            logger.warning(f"Initial connection failed: {e}. Retrying in 5s...")
            time.sleep(5)
    ...
    try:
        while True:
            heartbeat = {"client_id": CLIENT_ID, "timestamp": time.time()}
